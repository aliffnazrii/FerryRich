                        ...
                        <form class="row g-4" method="post" action="{{route('users.update', $user->id)}}">
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="role" class="form-control" id="role" required disabled>
                                    <option value="Content Creator" {{ (isset($user) && $user->role == 'Content Creator') ? 'selected' : '' }}>Content Creator</option>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" value="{{ $user->tiktok_username ?? '' }}" name="tiktok_username" class="form-control" id="tiktok_username" placeholder="TikTok Username">
                            </div>
                        
                            <div class="col-md-12">
                                <label for="tiktok_profile_link" class="form-label">TikTok Profile Link</label>
                                <input type="url" value="{{ $user->tiktok_profile_link ?? '' }}" name="tiktok_profile_link" class="form-control" id="tiktok_profile_link" placeholder="TikTok Profile Link">
                            </div>
                        
                            <div class="col-md-12">
                                <label for="ic_number" class="form-label">IC Number</label>
                                <input type="text" value="{{ $user->ic_number ?? '' }}" name="ic_number" class="form-control" id="ic_number" placeholder="IC Number">
                            </div>
                        
                            <div class="col-md-12">
                                <label for="bank_name" class="form-label">Bank Name</label>
                                <input type="text" value="{{ $user->bank_name ?? '' }}" name="bank_name" class="form-control" id="bank_name" placeholder="Bank Name">
                            </div>
                        
                            <div class="col-md-12">
                                <label for="cardholder_name" class="form-label">Cardholder Name</label>
                                <input type="text" value="{{ $user->cardholder_name ?? '' }}" name="cardholder_name" class="form-control" id="cardholder_name" placeholder="Cardholder Name">
                            </div>
                        
                            <div class="col-md-12">
                                <label for="bank_account_number" class="form-label">Bank Account Number</label>
                                <input type="text" value="{{ $user->bank_account_number ?? '' }}" name="bank_account_number" class="form-control" id="bank_account_number" placeholder="Bank Account Number">
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <select name="is_approved" class="form-control" id="is_approved" required>
                                    <option value="1" {{ (isset($user) && $user->is_approved) ? 'selected' : '' }}>Approved</option>
                                    <option value="0" {{ (isset($user) && !$user->is_approved) ? 'selected' : '' }}>Pending</option>
                                </select>
                            </div>
                        ...
                        <div class="full-info">
                            <div class="info-list d-flex flex-column gap-3">
                                ...
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">flag</span>
                                    <p class="mb-0">Country: {{ $user->country ?? 'No Details' }}</p> </div>
                                ...
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">tiktok</span>
                                    <p class="mb-0">TikTok Username: {{ $user->tiktok_username ?? 'No Details' }}</p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">link</span>
                                    <p class="mb-0">TikTok Profile Link: <a href="{{ $user->tiktok_profile_link ?? '#' }}" target="_blank">{{ $user->tiktok_profile_link ?? 'No Details' }}</a></p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">credit_card</span>
                                    <p class="mb-0">IC Number: {{ $user->ic_number ?? 'No Details' }}</p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">account_balance</span>
                                    <p class="mb-0">Bank Name: {{ $user->bank_name ?? 'No Details' }}</p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">person</span>
                                    <p class="mb-0">Cardholder Name: {{ $user->cardholder_name ?? 'No Details' }}</p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">account_balance</span>
                                    <p class="mb-0">Bank Account Number: {{ $user->bank_account_number ?? 'No Details' }}</p>
                                </div>
                                <div class="info-list-item d-flex align-items-center gap-3"><span class="material-icons-outlined">verified</span>
                                    <p class="mb-0">Approval Status: {{ $user->is_approved ? 'Approved' : 'Pending' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
